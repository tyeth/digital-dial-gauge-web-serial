...
def get_gauge_values(port_path, count=0, timeout=0, log_func=None):
    import serial, time
    ser = serial.Serial(
        port=port_path,
        baudrate=9600,
    ...
    def default_log_line(msg):
        ...
        print(line)
    log_line = log_func or default_log_line
    ...
    def parse_gauge_data(
        ...
        nonlocal serial_buffer, pending_minus
        log_raw = log_raw or log_line
        log_bin = log_bin or log_line
        log_parsed = log_parsed or log_line
        log_info = log_info or log_line
        log_warn = log_warn or log_line
        log_error = log_error or log_line
    ...
    def on_value_callback(mm_val, num_str):
        ...
        log_line(f"[VALUE] {mm_val} mm")

    log_line(f"[STATUS] Connected to {port_path}")
    start_time = time.time()
    try:
        while True:
            if timeout and (time.time() - start_time) > timeout:
                log_line(f"[TIMEOUT] Timeout reached after {timeout} seconds.")
                break
            ...
            if count and len(collected_values) >= count:
                log_line(f"[DONE] Collected {len(collected_values)} value(s). Exiting.")
                break
    except KeyboardInterrupt:
        log_line('[STATUS] Disconnected')
    finally:
        ser.close()
...
def main():
    ...
    values = get_gauge_values(args.port, args.count, args.timeout)
